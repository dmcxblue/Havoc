...
DWORD createTask(const wchar_t * server, wchar_t * taskpath, const wchar_t* xmldef, int mode, BOOL force)
{
	...
	if (force)
	{
		hr = pRootFolder->lpVtbl->RegisterTask(pRootFolder, BSTRtaskname, BSTRtaskxml, TASK_CREATE_OR_UPDATE, Vthisuser, VNull, taskType, VNull, &pRegisteredTask);
		if(FAILED(hr))
		{
	...
	else // else create only
	{
		...
		hr = pRootFolder->lpVtbl->RegisterTask(pRootFolder, BSTRtaskname, BSTRtaskxml, TASK_CREATE, Vthisuser, VNull, taskType, VNull, &pRegisteredTask);
		if(FAILED(hr))
		{
			internal_printf("Failed to register task (%lX)\n", hr);
			goto createTask_end;
